...
namespace Compass.CommomLibrary.PatamarDat
{
    public class PatamarDat : BaseDocument
    {
        ...
        public UNSBlock Nao_Simuladas { get { return (UNSBlock)blocos["Nao_Simuladas"]; } }
        ...
        public PatamarDat(string filepath)
        {
            ...
            using (var tr = new System.IO.StreamReader(fs))
            {
                ...
                do
                {
                    if (tr.EndOfStream && (line == null || line =="9999"))
                    {
                        break;
                    }
                    var newLine = Blocos["Nao_Simuladas"].CreateLine(line);

                    if (newLine is UNSABLine)
                    {
                        newLine["Submercado"] = Blocos["Nao_Simuladas"].Last()["Submercado"];
                        newLine["Tipo_Usina"] = Blocos["Nao_Simuladas"].Last()["Tipo_Usina"];
                        newLine["Patamar"] = patT;
                        if (!(newLine["Ano"] is int)) newLine["Ano"] = Blocos["Nao_Simuladas"].Last()["Ano"];

                        if (++patT > nPat) patT = 1;
                    }

                    Blocos["Nao_Simuladas"].Add(newLine);
...
";
    ...
    public class UNSBlock : BaseBlock<UNSLine>
    {
        string header =
@" SBM  BLOCO
 XXX XXX
...
";
        ...
        public override UNSLine CreateLine(string line = null)
        {
            line = (line ?? "");
            var id = line.Trim().Split(' ')[0];
            ...
            if (id.Length < 4)
            {
                return BaseLine.Create<UNSLine>(line);
            }
            else
            {
                return BaseLine.Create<UNSABLine>(line);
            }
        }
    ...
    public class UNSLine : BaseLine
    {
        public static readonly BaseField[] campos = new BaseField[] {
                new BaseField( 2 , 4 ,"I3"  , "Submercado"),
                new BaseField( 5 , 7 ,"I3"  , "Tipo_Usina"),
        };
        ...
        public int? Ano
        {
            get
            {
                if (this is UNSABLine) return this[0];
                else return null;
            }
            set { this[0] = value; }
        }
        ...
        public int? Patamar
        {
            get
            {
                if (this is UNSABLine) return this["Patamar"];
                else return null;
            }
            set { this["Patamar"] = value; }
        }
    ...
    public class UNSABLine : UNSLine
    {
        public static readonly new BaseField[] campos = new BaseField[] {
                new BaseField( 4 , 7 ,"I4"  , "Ano"),
                new BaseField( 9  , 14,"F6.4"  ,  "Fator 1"),
                new BaseField( 16 , 21,"F6.4"  ,  "Fator 2"),
                new BaseField( 23 , 28,"F6.4"  ,  "Fator 3"),
                new BaseField( 30 , 35,"F6.4"  ,  "Fator 4"),
                new BaseField( 37 , 42,"F6.4"  ,  "Fator 5"),
                new BaseField( 44 , 49,"F6.4"  ,  "Fator 6"),
                new BaseField( 51 , 56,"F6.4"  ,  "Fator 7"),
                new BaseField( 58 , 63,"F6.4"  ,  "Fator 8"),
                new BaseField( 65 , 70,"F6.4"  ,  "Fator 9"),
                new BaseField( 72 , 77,"F6.4"  ,  "Fator 10"),
                new BaseField( 79 , 84,"F6.4"  ,  "Fator 11"),
                new BaseField( 86 , 91,"F6.4"  ,  "Fator 12"),
                new BaseField( 0 , 0 ,"I3"  , "Submercado"),
